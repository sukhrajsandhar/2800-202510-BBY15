...
// Add event listeners
document.getElementById('addCampsiteBtn').addEventListener('click', () => {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?access_token=${window.mapboxgl.accessToken}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.features && data.features.length > 0) {
                            if (window.geocoderInput) {
                                window.geocoderInput.value = data.features[0].place_name;
                            }
                        }
                    });
            },
...
// Unified geocoder and geolocation for location input
function initializeGeocoder() {
    ...
    geocoder.on('result', (event) => {
        selectedCoordinates = event.result.center;
        console.log('Geocoder selected coordinates:', selectedCoordinates);
    ...
    window.geocoderInput = document.querySelector('#geocoder input[type="text"]');
}

// Handle form submission
document.getElementById('saveCampsite').addEventListener('click', async () => {
    console.log('Save button clicked. selectedCoordinates:', selectedCoordinates);
    ...
    const location = JSON.parse(document.getElementById('campsiteLocation').value);
    ...
    const campsiteData = {
        ...
        place_name: location.place_name, // send address if available
        ratings: [{
            rating: initialRating,
            timestamp: new Date().toISOString(),
            userId: 'current-user' // Replace with actual user ID when authentication is implemented
        }],
        averageRating: initialRating,
        totalRatings: 1,
        reservation: '' // or get from form if you have this field
    };
    ...
    try {
        ...
        if (response.ok) {
            ...
            throw new Error('Failed to save campsite');
        }
    } catch (error) {
